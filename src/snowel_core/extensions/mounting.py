...
def _load_hooks(manifest: PackManifest) -> tuple[list, list]:
    """hooks 规则加载占位：T2 阶段恒空，T3 接线 load_hooks 真实现（R5：
    hooks 失败不阻断挂载，警告随 mount 响应返回）。"""
    return [], []


def mount(api, name: str) -> dict:
    """挂载扩展包（R5 顺序）：定位 → 组模型构建全成功 → 单事务 append
    extension_mounted + apply 投影 → 进程态注册组 → hooks（T3 接线）。"""
    conn = api._conn
    ...
    _, hook_warns = _load_hooks(manifest)
...
def reload(api) -> list[str]:
    ...
    正常 → 构建组模型并注册进程态。整体吞错转警告，绝不让 open 崩。
