...
- 当前阶段：**v1.0.0 收尾——Step 1–6 全部完成合入 dev-1.0.0（末位 aeb988d）+ Step 7 覆盖终核完成（2362e33，83 例全锚）**。**Step 7 人工走查进行中**（2026-08-29 本机 Windows 部署就绪：L16 扩展加载崩经 python.org 构建实证证伪关闭、web/dist 构建绿；走查清单见 handoff §3，发现项记清单后按 dev-workflow 开修复波）→ Step 8 发布（RW-① mcp>=1.29 下限 + Release 打包，均需发话）；新会话执行入口：`docs/v1.0.0/details/handoff-v1-final.md`（工作交接总入口）+ 各 step ledger。
