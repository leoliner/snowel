...
| `web/`（vitest：App / sse / components/*.test.tsx） | Web 前端（React 三栏界面；vitest + @testing-library/react，vi.mock 数据层不发真实请求） | TC-SH-08 组件锚（浏览器级 E2E 豁免 W3）、TC-SH-09 组件锚（InspirationPanel / GenerateForm derive_from / App 挂载）、TC-SH-10 组件锚（ProseEditor 拍侧栏合并/删除/拒绝红条）、TC-SH-13 组件锚（ManualModal："？"入口/目录 12 章/关键词过滤/Markdown 渲染/esc 与遮罩关闭）、TC-SH-14 组件锚（Tour：欢迎卡不遮工作区/开始导览/前进后退/跳过与完成 localStorage 记忆/手册"重看导览"重启） |
| `retrieval/test_context.py` | `retrieval/`（compose_context/审计/hybrid） | TC-RT-01/02/03、TC-ON-02（FTS 面） |
| `retrieval/test_rewrite.py` | `retrieval/`（rewrite_query 检索改写前置：三路入口并轨、审计 `rewrite` 键/`rewrite_failed` 标记、开关关闭早退） | TC-RT-07（改写生效见于审计 / 失败回落+标记 / 关闭纯确定性） |
